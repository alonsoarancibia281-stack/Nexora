...
create table public.profiles (
 ...
 first_name text not null, last_name text not null, country text not null,
...
('free','daily_analyses','3'),('free','active_alerts','1'),('free','ai','false'),('essential','daily_analyses','25'),('essential','active_alerts','20'),('essential','ai','false'),('pro','daily_analyses','100'),('pro','active_alerts','-1'),('pro','ai','true'),('elite','daily_analyses','-1'),('elite','active_alerts','-1'),('elite','ai','true'),('owner','daily_analyses','-1'),('owner','active_alerts','-1'),('owner','ai','true'),('owner','admin','true');
...
create policy subscriptions_self_read on public.subscriptions for select using(user_id=auth.uid() or public.is_admin());
create policy plans_authenticated_read on public.subscription_plans for select to authenticated using(true);
create policy features_authenticated_read on public.plan_features for select to authenticated using(true);
create policy consents_self_read on public.user_consents for select using(user_id=auth.uid() or public.is_admin());
create policy devices_self on public.user_devices for select using(user_id=auth.uid() or public.is_admin());
create policy flags_authenticated_read on public.feature_flags for select to authenticated using(true);
create policy audit_owner_read on public.audit_logs for select using(public.is_owner());
-- No client policies for verification_codes: service-role Edge Functions only.
