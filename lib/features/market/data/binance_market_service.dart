...
class BinanceMarketService {
  ...
  Future<List<MarketAsset>> loadUsdtMarket() async {
    ...
    if (response.statusCode != 200) throw Exception('No se pudo cargar Binance (${response.statusCode}).');
  ...
  Stream<double> livePrice(String symbol) {
    final stream = '${symbol.toLowerCase()}@trade';
    final channel = WebSocketChannel.connect(Uri.parse('$_ws/$stream'));
